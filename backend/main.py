...
async def chat_with_agent(request:QueryRequests):
    trace_event_for_frontend : List[TraceEvent]=[]
    
    try :
        # passing web info into the config for the agent to access
        config: Any = {
            "configurable": {
        ...
        inputs: AgentState = {"messages": [HumanMessage(content=request.query)]}
        ...
        # Ensure response is a string for the Pydantic model
        return AgentResponse(response=str(final_message), trace_events=trace_event_for_frontend)
